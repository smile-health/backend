...
export class AssetInventoryService {
  ...
  async sendMaintenanceReminder(apiContext?: Context): Promise<void> {
    await new TransactionManager(db).transaction(async (trx) => {
      ...
      await this.assetNotif.handleAssetMaintenanceReminder(
        customContext,
        context
      )
    })
  ...
  async sendCalibrationReminder(apiContext?: Context): Promise<void> {
    await new TransactionManager(db).transaction(async (trx) => {
      const translator = i18n.cloneInstance()
      translator.changeLanguage("id")
      const customContext = new CustomContext({
      ...
      await this.assetNotif.handleAssetCalibrationReminder(
        customContext,
        context
      )
    })
  ...
  async sendWarrantyReminder(apiContext?: Context): Promise<void> {
    await new TransactionManager(db).transaction(async (trx) => {
      const translator = i18n.cloneInstance()
      translator.changeLanguage("id")
      const customContext = new CustomContext({
      ...
      await this.assetNotif.handleAssetWarrantyReminder(customContext, context)
    })
  }
  ...
  async sendStatusChangedNotification(
    ...
    await new TransactionManager(db).transaction(async (trx) => {
      ...
      const context = apiContext || this.createFakeContext()
      await this.assetNotif.handleAssetChangedNotification(
        customContext,
        context,
        assetId
      )
