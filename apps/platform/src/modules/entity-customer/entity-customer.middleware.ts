...
export class EntityCustomerMiddleware {
  ...
  validateDeleteCustomer = createMiddleware(async (c, next) => {
    ...
    const { entity_id, entity_id_relation } = body
    await this.#validateEntityID(c, [entity_id, entity_id_relation])
  ...
  validateImportEntityCustomer = async (
    ...
    const [entityDetail, listCustomer] = await Promise.all([
      this.entityCustomerRepo.findEntityDetail(c, Number(id)),
      this.entityCustomerRepo.getListEntityCustomers(c, Number(id)),
    ])
    ...
    const [listEntity, listEntityActivities, listEntityCustomer] =
      await Promise.all([
        ...
        this.entityCustomerRepo.getValidateListEntityActivities(c, Number(id)),
