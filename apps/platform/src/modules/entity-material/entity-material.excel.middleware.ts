...
export class EntityMaterialExcelMiddleware {
    ...
    return GetTemplateEntityMaterialSchema.extend({
      ...
      province_id: generalMultipleIdSchema
        .superRefine(async (val, ctx) => {
          if (val.length > 0) {
            ...
            const isExist = val.every((item) => dataIds.includes(item))
            this.#isExist(isExist, ctx)
          }
        })
        .optional(),
      ...
      activity_id: generalMultipleIdSchema
        .superRefine(async (val, ctx) => {
          if (val.length > 0) {
            const ids = val.map((id) => Number(id))
            const data = await this.activityRepo.findByIds(c, ids)
            const dataIds = data.map((item) => item.id)
            const isExist = ids.every((item) => dataIds.includes(item))
            this.#isExist(isExist, ctx)
          }
  ...
  #getColumnTranslations = (c: Context): ColumnImportSchema => {
    return {
  ...
  #collectAndAssociate = (dataEntities, dataMaterialActivities) => {
    return {
  ...
  validateImport = async (
    ...
    rows.forEach((row, index) => {
      ...
      if (!materialActivitieIds.includes(row.materialActivityId)) {
        ...
        row.materialId =
          materialActivityAssociate[row.materialActivityId]
            ?.master_material_id ?? 0
      }
  ...
  logErrors = createMiddleware(async (c, next) => {
    ...
    if (c.var.errors) {
      ...
      console.log(JSON.stringify(notes).length)
