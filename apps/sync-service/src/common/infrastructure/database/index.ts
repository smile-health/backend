...
export const pool = createPool({
  ...
  // Performance & transaction optimizations
  enableKeepAlive: true,
  keepAliveInitialDelay: 0,
  maxIdle: 10,
  // Removed invalid mysql2 options: acquireTimeout, idleTimeout
  // (mysql2 uses 'timeout' for connection timeout)
})
...
testMySQLConnection()
  .then((result) => {
    if (result.success) {
      console.log(
        `🎉 [sync-service] Database connection verified: ${result.message}`
      )
    } else {
      console.error(
        `🚨 [sync-service] Database connection failed: ${result.message}`
      )
    }
  })
  .catch((error) => {
    console.error(`🚨 [sync-service] Database connection test error:`, error)
  })
