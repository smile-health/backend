...
export const migrateEmonevRegencies = async (
  batchSize: number,
  programId = 1
) => {
  const startTime = new Date()
  console.info(`Migration Emonev Regencies started at: ${startTime.toLocaleString()}`)
  ...
  try {
    while (true) {
      const rows = await migrationDB
        .selectFrom("integration_emonev_regencies as ier")
        .select(["ier.id"])
        .orderBy("ier.id")
        .limit(batchSize)
      ...
      const regencyIds = collect(rows, "id")

      await db.transaction().execute(async (trx) => {
        const count = await doMigrateEmonevRegencies(trx, migrationDB, regencyIds)
        totalCount += count
      })

      page++
      console.log(`Emonev Regencies batch ${page} completed (${rows.length} records)`);
    }

    console.info(`✅ Emonev Regencies migration completed: ${totalCount} records`)
    return { count: totalCount }
  } catch (error) {
    console.error("❌ Emonev Regencies migration failed")
    console.error(error)
    throw error
  }
}

export const doMigrateEmonevRegencies = async (
  trx: Transaction<DB>,
  ...
  const insertData = regencies.map((regency) => ({
    id: regency.id,
  ...
  await trx
    .insertInto("integration_emonev_regencies")
    .values(insertData)
