...
class StreamingCSVWriter {
  ...
  async init() {
    ...
    const header1 =
      "Source ID,Source Name,Source Code,Source Province,Source Regency,Source Sub District,Source Transaction Count,Target ID,Target Name,Target Code,Target Province,Target Regency,Target Sub District,Target Transaction Count,Similarity Score\n"
    const header2 =
      "Source ID,Source Province ID,Source Regency ID,Source Sub District ID,Source Transaction Count,Target ID,Target Province ID,Target Regency ID,Target Sub District ID,Target Transaction Count,Similarity Score\n"
...
/**
 * 🔥 MODE: LOCALITY-BASED COMPARISON (MEMORY OPTIMIZED) ⭐ UPDATED
 */
 ...
 */
export const populateNamingOfTheSameEntityImun = async (
  limitEntities: number | null = null,
  localityFilter: "province" | "regency" | null = "regency",
  useStreamingCSV: boolean = false
) => {
  console.log(
    "\n🚀 Starting Entity Similarity Detection (Memory Optimized with Transaction Count)..."
  )
  ...
  console.log(`\n${"=".repeat(70)}`)
  console.log(
    "ENTITY SIMILARITY DETECTION (MEMORY OPTIMIZED + TRANSACTION COUNT)"
  )
